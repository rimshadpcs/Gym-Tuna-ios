...
// MARK: - WorkoutScreen with Existing ViewModel
struct WorkoutScreenWithExistingViewModel: View {
    @Environment(\.themeManager) private var themeManager
    ...
    // State for UI controls
    @State private var initialized = false
    @State private var currentActiveSetIndex: (Int, Int)? = nil
    @State private var showFinishDialog = false
    @State private var showQuickWorkoutFinishDialog = false
    @State private var showDiscardConfirmationDialog = false
    @State private var workoutCompletionStatus: WorkoutCompletionStatus? = nil
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // THEN check for pending exercises from ExerciseChannel
            DispatchQueue.main.async {
                print("🏋️ WORKOUT SCREEN WITH EXISTING VM: About to call checkForPendingExercises...")
                viewModel.checkForPendingExercises()
                print("🏋️ WORKOUT SCREEN WITH EXISTING VM: checkForPendingExercises call completed")
            }
        }
        .onReceive(viewModel.$lastExerciseOperation) { operation in
            print("🎉 WORKOUT SCREEN WITH EXISTING VM: Received exercise operation update: \(operation?.message ?? "nil")")
            guard let operation = operation else { 
                print("🎉 WORKOUT SCREEN WITH EXISTING VM: Operation is nil, ignoring")
                return 
            }
            
            print("🎉 WORKOUT SCREEN WITH EXISTING VM: Setting feedback message: \(operation.message)")
            exerciseFeedbackMessage = operation.message
            withAnimation {
                showExerciseFeedback = true
                print("🎉 WORKOUT SCREEN WITH EXISTING VM: Showing feedback toast")
            }
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
                withAnimation {
                    showExerciseFeedback = false
                    print("🎉 WORKOUT SCREEN WITH EXISTING VM: Hiding feedback toast")
                }
            }
        }
        .sheet(isPresented: $showFinishDialog) {
            if let status = workoutCompletionStatus {
                FinishWorkoutBottomSheet(
                    ...
                    onConfirm: {
                        viewModel.finishWorkout(forceFinish: false) {
                            print("✅ WorkoutScreenWithExistingViewModel: finishWorkout success callback - closing dialog and calling onFinish")
                            showFinishDialog = false
                            onFinish()
                        }
                    },
                    onForceFinish: {
                        viewModel.finishWorkout(forceFinish: true) {
                            print("✅ WorkoutScreenWithExistingViewModel: finishWorkout(forceFinish) success callback - closing dialog and calling onFinish")
                            showFinishDialog = false
                            onFinish()
                        }
                    },
        ...
        .sheet(isPresented: $showQuickWorkoutFinishDialog) {
            if let status = workoutCompletionStatus {
                QuickWorkoutFinishBottomSheet(
                    ...
                    isPremium: viewModel.isPremium,
                    onJustFinish: {
                        print("📝 WorkoutScreenWithExistingViewModel: Quick workout onJustFinish - showing finish dialog")
                        showQuickWorkoutFinishDialog = false
                        showFinishDialog = true
                    },
                    onSaveAndFinish: { routineName in
                        viewModel.saveAsRoutine(
                            routineName: routineName,
                            onSuccess: {
                                print("💾 WorkoutScreenWithExistingViewModel: Save routine success - showing finish dialog")
                                showQuickWorkoutFinishDialog = false
                                showFinishDialog = true
                            },
                            onUpgradeRequired: {
    ...
    // MARK: - View Components (reused from main WorkoutScreen)
            ...
            LazyVStack(spacing: 12) {
                ForEach(Array(viewModel.exercises.enumerated()), id: \.element.exercise.id) { exerciseIndex, exercise in
                    ...
                    ExerciseCard(
                        ...
                        onReplaceExercise: { workoutExercise in
                            print("🔄 WORKOUT SCREEN WITH EXISTING VM: Replace exercise button tapped for: \(workoutExercise.exercise.name)")
                            viewModel.replaceExercise(workoutExercise)
                            print("🔄 WORKOUT SCREEN WITH EXISTING VM: About to call navigation callback...")
                            onReplaceExercise()
                            print("🔄 WORKOUT SCREEN WITH EXISTING VM: Navigation callback completed")
                        },
                        onAddToSuperset: { workoutExercise in
                            viewModel.addToSuperset(workoutExercise)
                        },
    ...
    private func initializeWorkout() {
        if !initialized {
            ...
            print("🎯 WorkoutScreenWithExistingViewModel initializeWorkout:")
            print("   routineId: \(routineId ?? "nil")")
            print("   routineName: \(routineName)")
            print("   sessionState exists: \(sessionState != nil)")
            if let sessionState = sessionState {
            ...
            print("⚠️ WorkoutScreenWithExistingViewModel initializeWorkout SKIPPED - already initialized")
        }
    }
